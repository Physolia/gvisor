...
func (l *Lifecycle) StartContainer(args *StartContainerArgs, _ *uint32) error {
	...
	startSigForwarding := false
	if l.Kernel.GlobalInit() == nil {
		startSigForwarding = true
	}
	...
	if startSigForwarding {
		// Ensure that signals received in sentry context are forwarded to the
		// emulated kernel (except for the panic signal, which should trigger a
		// panic).
		l.stopSigForwarding = sighandling.StartSignalForwarding(func(sig linux.Signal) {
			if sig == kernel.SignalPanic {
				panic(fmt.Sprintf("Signal-induced panic due to signal: %s", unix.Signal(sig)))
			}

			l.Kernel.SendExternalSignal(&linux.SignalInfo{Signo: int32(sig)}, "sentry")
		})
	}
