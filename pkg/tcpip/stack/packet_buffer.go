...
// PacketBufferPtr is a "smart" pointer to a PacketBuffer.
...
//
// +stateify savable
// TODO: typedef as PacketBufferPtr?
// type PacketBuffer struct {
// 	*PacketBufferInner
// 	ticket Ticket
// }

// PacketBufferInner needs a comment to make the linter happy.
type PacketBufferInner struct {
	_ sync.NoCopy
...
func NewPacketBuffer(opts PacketBufferOptions) PacketBufferPtr {
	pk := pkPool.Get().(*PacketBufferInner)
	pk.reset()
	...
	return PacketBufferPtr{
		PacketBufferInner: pk,
		ticket:            pk.refs.InitRefs(),
	}
}

// IncRef increments the PacketBuffer's refcount.
// TODO: The return value should be used.
func (pk PacketBufferPtr) IncRef() PacketBufferPtr {
	return PacketBufferPtr{
		PacketBufferInner: pk.PacketBufferInner,
		ticket:            pk.refs.IncRef(),
	}
}
...
func (pk PacketBufferPtr) DecRef() {
	// Note that we avoid allocating a closure by having destroy take a
	// PacketBufferPtr rather than capturing pk.
	pk.refs.DecRef(pk.ticket, destroy, pk)
}

// Separated out for testing.
var destroy = func(pk PacketBufferPtr) {
	pkPool.Put(pk.PacketBufferInner)
}

func (pk *PacketBufferInner) reset() {
	*pk = PacketBufferInner{}
}
...
func (pk PacketBufferPtr) Views() []tcpipbuffer.View {
...
func (pk PacketBufferPtr) push(typ headerType, size int) tcpipbuffer.View {
	h := &pk.headers[typ]
	if h.length > 0 {
...
func (pk PacketBufferPtr) consume(typ headerType, size int) (v tcpipbuffer.View, consumed bool) {
	h := &pk.headers[typ]
	if h.length > 0 {
...
func (pk PacketBufferPtr) headerView(typ headerType) tcpipbuffer.View {
	h := &pk.headers[typ]
	if h.length == 0 {
		return nil
	}
...
func (pk PacketBufferPtr) Clone() PacketBufferPtr {
	newPk := pkPool.Get().(*PacketBufferInner)
	newPk.reset()
	...
	return PacketBufferPtr{
		PacketBufferInner: newPk,
		ticket:            newPk.refs.InitRefs(),
	}
}
...
func (pk *PacketBufferPtr) IsNil() bool {
	return pk.PacketBufferInner == nil
}
